...
export default {
    ...
    onSubmit(formName) {
       this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            let _this = this
            this.$http.post('editAtricle',this.form).then(res=>{
              this.$message({
                type: 'success',
                message: '修改成功'
              });   
  ...
  mounted(){
      ...
      this.$http.post('/getAtricleDetail',{id:_this.$route.query.id}).then(res=>{
          this.form.atricle_name = res.data[0].atr_name;
          this.form.atricle_title = res.data[0].atr_title;
          this.form.region = res.data[0].type;
          this.form.date = res.data[0].time;
          this.form.content = res.data[0].content;
          this.form.id = _this.$route.query.id;
      })
  }
}
</script>
